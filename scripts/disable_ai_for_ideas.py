...
`ai_will_do` block found in the `common/ideas` .txt files.
...
navigates to every `ai_will_do` block (via its `ideas -> <category> -> <idea>`
path) and creates:
...
Usage:
    python gen_sandbox_ai_will_do_includes.py <in_dir> <out_dir>

Defaults, if omitted:
    in_dir  = "Hearts of Iron IV/common/ideas"
    out_dir = "out"
"""
...
def find_ai_will_do_paths(text):
    """
    For every idea entry with a direct `ai_will_do` block, return a tuple:

        (cat_name, cat_index, idea_name, idea_index)

    where `cat_index` is the 0-based position of the category among same-named
    categories under `ideas` (or None if unique), and `idea_index` is the
    analogous position of the idea among same-named entries in its category (or
    None if unique). Indices let duplicated names resolve to the intended block
    via the engine's [N] selector; a bare name always resolves to the first
    match, which is wrong when a name repeats.

    Structure assumed (vanilla common/ideas):
        ideas = {
            <category> = {
                <idea> = {
                    ...
    ...
    """
    paths = []
    for name, o, c in _iter_named_blocks(text, 0, len(text)):
        if name != "ideas":
            continue

        cat_blocks = list(_iter_named_blocks(text, o, c))
        cat_name_counts = {}
        for cn, _, _ in cat_blocks:
            cat_name_counts[cn] = cat_name_counts.get(cn, 0) + 1

        cat_seen = {}
        for cat_name, cat_o, cat_c in cat_blocks:
            cat_pos = cat_seen.get(cat_name, 0)
            cat_seen[cat_name] = cat_pos + 1
            cat_index = cat_pos if cat_name_counts[cat_name] > 1 else None

            idea_blocks = list(_iter_named_blocks(text, cat_o, cat_c))
            idea_name_counts = {}
            for inm, _, _ in idea_blocks:
                idea_name_counts[inm] = idea_name_counts.get(inm, 0) + 1

            idea_seen = {}
            for idea_name, idea_o, idea_c in idea_blocks:
                idea_pos = idea_seen.get(idea_name, 0)
                idea_seen[idea_name] = idea_pos + 1
                if not _has_direct_child(text, idea_o, idea_c, "ai_will_do"):
                    continue
                idea_index = (idea_pos
                              if idea_name_counts[idea_name] > 1 else None)
                paths.append((cat_name, cat_index, idea_name, idea_index))
    return paths
...
def build_include(paths):
    ...
    (cat_name, cat_index, idea_name, idea_index) tuples, grouping ideas under
    their shared category header. A `[N]` selector is emitted for any name that
    is not unique among its siblings."""
    lines = ["ideas:"]

    # Group ideas by category identity (name + positional index), preserving
    # first-seen order.
    grouped = {}
    order = []
    for cat_name, cat_index, idea_name, idea_index in paths:
        key = (cat_name, cat_index)
        if key not in grouped:
            grouped[key] = []
            order.append(key)
        grouped[key].append((idea_name, idea_index))

    for key in order:
        cat_name, cat_index = key
        lines.append(f"{INDENT}{_header(cat_name, cat_index)}:")
        for n, (idea_name, idea_index) in enumerate(grouped[key]):
            if n > 0:
                # Blank line between idea blocks (not before the first).
                lines.append("")
            lines.append(f"{INDENT * 2}{_header(idea_name, idea_index)}:")
            lines.append(f"{INDENT * 3}ai_will_do:")
            for ml in _MODIFIER_LINES:
                lines.append(f"{INDENT * 4}{ml}")
...
def main():
    in_dir = sys.argv[1] if len(sys.argv) > 1 else "Hearts of Iron IV/common/ideas"
